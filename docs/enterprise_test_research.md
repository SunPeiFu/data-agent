...
| 字段级影响分析 | `test_column_level_impact_analysis_uses_field_granularity` |
...
| 表名/字段名需要后续元数据校验 | `test_policy_resolver_marks_table_and_field_as_metadata_validation_candidates` |

## 测试原则

- 不只测 intent，还要测 `task_steps` 是否路由到正确工具。
- 表名和字段名只作为候选值，最终必须由元数据服务确认。
- column-level 场景必须生成 `lineage_granularity=field`。
